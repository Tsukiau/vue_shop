      ...
      <el-table border stripe :data="rolesList">
            ...
            <el-row :class="['bbottom', index === 0 ? 'btop' : '']" v-for="(item, index) in scope.row.children"
              ...
              <el-col :span="19">
                <el-row :class="[index2 === 0 ? '' : 'btop']" v-for="(item2, index2) in item.children" :key="item2.id"
                  ...
                  <!-- 二级 -->
                  <el-col :span="6">
                    <el-tag @close="delTagId(scope.row,item2.id)" closable
                  ...
                  <el-col :span="18">
                    <el-tag
                    @close="delTagId(scope.row,item3.id)" closable type="warning" v-for="(item3) in item2.children"
        ...
        <el-table-column label="操作" width="300px">
          <template>
            ...
            <el-button type="warning" icon="el-icon-setting" size="mini" @click="showSetDial">分配权限 </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
