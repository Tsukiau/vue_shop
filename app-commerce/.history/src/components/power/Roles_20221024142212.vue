        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row :class="['bbottom', index === 0 ? 'btop' : '']" v-for="(item, index) in scope.row.children"
              ...
              <!-- 二级和三级 -->
              <el-col :span="19">
                <el-row :class="[index2 === 0 ? '' : 'btop']" v-for="(item2, index2) in item.children"
                :key="item2.id" class="flex_center">
                  <!-- 二级 -->
                  <el-col :span="6">
                  ...
                  <!-- 三级 -->
                  <el-col :span="18">
                    <el-tag @close="delTagId(item3。id)" closable type="warning" v-for="(item3) in item2.children" :key="item3.id">{{ item3.authName }}</el-tag>
                  </el-col>
                </el-row>
              </el-col>
            </el-row>
