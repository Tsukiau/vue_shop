<template>
  <div>
    ...
    <el-card>
      ...
      <el-table border stripe :data="rolesList">
        ...
        <el-table-column label="操作" width="300px">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="exaForm(scope.row.id)">编辑</el-button>
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="delForm(scope.row.id)">删除</el-button>
            <el-button type="warning" icon="el-icon-setting" size="mini" @click="showSetDialog(scope.row)">分配权限
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 分配权限对话框 -->
    <el-dialog title="分配权限" :visible.sync="showSetDialogVisible" width="50%" @close="SetRightDialogVisibleClosed">
      <el-tree :data="rightList" :props="treeAuthName" show-checkbox node-key="id" :default-expand-all="true"
    ...
    <el-dialog title="添加角色" :visible.sync="addDialogVisible" width="50%">
      <el-form ref="form" label-width="80px" v-model="addForm">
    ...
    <!-- 编辑角色 -->
    <el-dialog title="添加角色" :visible.sync="comDialogVisible" width="50%">
      <el-form ref="form" label-width="80px" v-model="comForm">
        <el-form-item label="角色名称" prop="roleName">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="comDialogForm(comForm.roleId)">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import { rolesApi, delRightsApi, rightListApi, addFormApi, delFormApi, exaFormApi, comFormApi } from '@/api/power'
...
export default {
  ...
  methods: {
    ...
    async addDialogForm() {
      try {
        const { data } = await addFormApi(this.addForm)
        console.log(data)
      } catch (err) {
    ...
    // 编辑角色
    async exaForm(id) {
      const { data } = await exaFormApi(id)
      this.comForm = data.data
      console.log(this.comForm)
      this.comDialogVisible = true
    },
    async comDialogForm(id) {
      const { data } = await comFormApi(id, this.comForm)
      console.log(data)
      this.getRolesApi()
    },
    // 删除角色
