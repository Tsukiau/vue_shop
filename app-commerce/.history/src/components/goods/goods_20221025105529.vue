      ...
      <el-table border stripe :data="goodsList">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button type="danger" @click="delId(scope.row.goods_id)" icon="el-icon-delete" size="mini"></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
<script>
import { goodsApi, delCategories } from '@/api/goods'
...
export default {
  ...
  methods: {
    ...
    async delId(id) {
      ...
      if (res !== 'confirm') {
        return this.$message.info('已经取消yyiyyyyyyyyyyiyiyyiiyiyiyiyiyiyiyyiiyyiiyiiyyiiyiyyiyiyiyiyiyiyiyiiyyiyiyiyiyiyiyiyiiyiyiyiyyi')
      }

      try {
        const { data } = await delCategories(id)
        console.log(data)
